...
class PandasBaseTransformer:
    ...
    def concatenate_dataframes(dataframes: list, join="outer") -> pd.DataFrame:
        ...
        try:
            ...
            # Filtra los DataFrames para excluir los vacíos o los que tienen solo valores NaN en todas sus columnas
            dataframes_filtrados = [
                df for df in dataframes if not df.empty and not df.isna().all().all()
            ]

            # Concatenar los DataFrames filtrados
            df_concatenado = pd.concat(dataframes_filtrados, join=join, ignore_index=True)
        ...
        for columna in columnas:
            try:
                ...
                if df[columna].isna().any():
                    logger.warning(
                        f"Algunos valores en la columna '{columna}' no se pudieron convertir a fecha y se establecieron como NaT."
                    )
                else:
                    logger.info(
                        f"Columna '{columna}' convertida a formato de fecha correctamente"
                    )

            except Exception as e:
                logger.critical(
                    f"Error crítico al convertir la columna '{columna}' a fecha: {e}"
                )
    ...
    def Filtrar_por_operacion(
        ...
        if (
            operacion in ["mayor", "menor", "mayor_igual", "menor_igual"]
            and valor_umbral is None
        ):
            raise ValueError(
                "Debes proporcionar 'valor_umbral' para la operación seleccionada."
            )
        if operacion == "entre_a_b_valores":
            if valor_min is None or valor_max is None:
                raise ValueError(
                    "Debes proporcionar 'valor_min' y 'valor_max' para la operación 'entre_a_b_valores'."
                )
